...
#define tam1 5
			...
			opcao2=0;
				...
				if(strcmp(tentativas,palavra[palavrasorteada])==0){
					printf("\n");
						...
						printf("\t\t\t\tPARABÉNS, VOCÊ ACERTOU E CONTABILIZOU +1 PONTO!!\n");
						printf("\t\t\t\t\t\t .-=========-.\n");
		              printf("\t\t\t\t\t\t\\ '-=======-'/\n");
		              printf("\t\t\t\t\t\t  |   .=.   |\n");
		    		 printf("\t\t\t\t\t\t((|  {{1}}  |))\n");
	...
	void instrucoes()
	{
		...
		gotoxy(39,15);printf("Digite a palavra toda em Maiuscúlo para o programa reconhece - la\n");	
		
			
		gotoxy(40,20);printf("Aperte Esc para sair a qualquer momento!\n");
	}
	        ...
	        do{
				...
				if(kbhit){tecla=getch();}//código pra fazer a setinha se movimentar entre as 5 escolhas
					...
					case 5:
						...
						gotoxy(50,19);printf("insira uma opção correta\n");
						gotoxy(50,21);scanf("%d",&op);
